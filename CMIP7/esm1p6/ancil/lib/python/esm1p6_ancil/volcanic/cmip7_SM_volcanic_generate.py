...
This script generates the CMIP7 ScenarioMIP volcanic ancillary file by
calculating the average stratospheric aerosol optical depth (SAOD) for each
historical month.
The SAOD is calculated by averaging extinction over latitude and summing over
stratospheric layers.
The resulting SAOD is saved to the specified save file.
The script takes command line arguments for the dataset version, dataset date
range, and save filename.
...
"""
TODO: Are these the same as in cmip7_SM.py? If so, we should use those instead
of duplicating the values here.
"""
...
def parse_args():
    """
    Parse the command line arguments for CMIP7 ScenarioMIP volcanic ancil file
    generation.
    """
    ...
    """
    --dataset-version and --dataset-vdate are already included in the
    dataset_parser() parent parser.
    """
