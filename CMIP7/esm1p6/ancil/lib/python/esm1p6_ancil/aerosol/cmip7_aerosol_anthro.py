...
Anthropogenic aerosol emissions refers to aerosol emissions from human
activities, such as industrial processes, transportation, and agriculture.

This module provides functions to load and interpolate CMIP7 anthropogenic
aerosol emissions data to the ESM1.5 grid.
The functions load the CMIP7 anthropogenic aerosol emissions data, aggregate the
sector dimension by summing all sector contributions into a single field, and
interpolate the data to the ESM1.5 grid using the specified interpolation
scheme. The interpolated data is then saved to the specified directory path with
the appropriate STASH item for the ESM1.5 model.
"""
...
def _anthro_dirpath(args, variable):
    """
    Return the directory path to the CMIP7 anthropogenic aerosol emissions data
...
def cmip7_aerosol_air_anthro_filepath(args, species, date_range):
    """
    Return the file path to the CMIP7 air anthropogenic aerosol emissions data
...
def cmip7_aerosol_anthro_filepath(args, species, date_range):
    """
    Return the file path to the CMIP7 anthropogenic aerosol emissions data for
    the given species and date range.
    """
...
def load_cmip7_aerosol_anthro(args, species, date_range, constraint):
    """
    Load the CMIP7 anthropogenic aerosol emissions data for the given species
    and date range, and apply the given constraint.
    """
...
def load_cmip7_aerosol_air_anthro_list(
    ...
    """
    Load the CMIP7 anthropogenic aerosol air emissions data for the given
    species and date range list, and apply the given constraint.
    """
...
def load_cmip7_aerosol_anthro_list(args, species, date_range_list, constraint):
    """
    Load the CMIP7 anthropogenic aerosol emissions data for the given species
    and date range list, and apply the given constraint.
    """
    # Load the CMIP7 anthropogenic aerosol air emissions data
    cube = load_cmip7_aerosol_list(
        args,
...
def cmip7_aerosol_anthro_interpolate(
    ...
    """
    Aggregate the sector dimension by summing all sector contributions into a
    single field.
    """
    cube_tot = cube.collapsed(["sector"], iris.analysis.SUM)
    """
    Interpolate the CMIP7 anthropogenic aerosol emissions data to the ESM1.5
    grid.
    """
    ...
    zero_poles(esm_cube)
    """
    Set the STASH item for the output cube so that it can be correctly
    identified in the ESM1.5 model.
    """
    esm_cube.attributes["STASH"] = iris.fileformats.pp.STASH(
        model=1, section=0, item=stash_item
    )
    """
    Save the interpolated CMIP7 anthropogenic aerosol emissions data to the
