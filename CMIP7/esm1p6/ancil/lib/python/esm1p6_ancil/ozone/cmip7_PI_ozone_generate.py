'''
This script generates the CMIP7 pre-industrial ozone ancillary file. It loads the CMIP7 pre-industrial ozone data, fixes the coordinates to match the ESM1.5 grid, and saves the result as an ancillary file.
...
# This function is declared in every ozone script.
def parse_args():
    '''
    Parse the command line arguments for CMIP7 pre-industrial ozone ancil file generation.
    '''
...
def esm_pi_ozone_save_dirpath(args):
    '''
    Return the directory path to save the ESM1.5 pre-industrial ozone ancil file.
    '''
    return (
...
if __name__ == "__main__":
    ''' Generate the CMIP7 pre-industrial ozone ancillary file. '''
    args = parse_args()
