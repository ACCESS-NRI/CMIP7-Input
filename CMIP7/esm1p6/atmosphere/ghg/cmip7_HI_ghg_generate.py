...
def parse_args():
    parser = ArgumentParser(
            ...
            description=('Generate input files from '
                         'CMIP7 historical greenhouse gas forcings'))
...
def load_cmip7_hi_ghg_mmr(args, ghg):
    ...
    date_constraint = cmip7_pro_greg_date_constraint_from_years(
            CMIP7_GHG_HI_BEG_YEAR,
            CMIP7_GHG_HI_END_YEAR)
    ...
    for year in range(CMIP7_GHG_HI_BEG_YEAR, CMIP7_GHG_HI_END_YEAR + 1):
        year_constraint = cmip7_pro_greg_date_constraint_from_years(
                year,
                year)
...
def cmip7_hi_ghg_patch(ghg_mmr_dict):
    ...
    OLD_REAL_MISSING_DATA_INDICATOR = -32768.0
    ...
    namelist_rates = np.full(namelist_years.shape, OLD_REAL_MISSING_DATA_INDICATOR)
    ...
    new_namelist_filepath = (
            hi_ghg_namelist_filepath.with_suffix('.nml.patched'))
    parser.read(
            hi_ghg_namelist_filepath,
            patch_str_namelist,
            new_namelist_filepath)
