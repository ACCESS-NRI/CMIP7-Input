...
# /g/data/vk83/prerelease/configurations/inputs/access-esm1p6/modern/flat10/atmosphere/forcing/global.N96/2025.12.05/CO2_fluxes_flat10.anc
...
def parse_args():
    ...
    parser.add_argument(
        "--year-start",
        help="Initial year in file. Interpolation section will start from this year.",
        required=True,
        type=int
    )
    parser.add_argument(
...
def create_f10cdr_data(nyear_zero, esm_latitude, esm_longitude):
    ...
    yearly_vals = np.linspace(
            start=PG10_C,
            stop=-PG10_C,
            num=INTERPOLATION_NYEARS + 1
    )
    decline_section = np.zeros(
            shape=(INTERPOLATION_NYEARS * 12 + 1, esm_latitude.shape[0], esm_longitude.shape[0])
    )
    ...
    flat_minus10_section = np.full(
            shape=(FLAT_MINUS10_NYEARS * 12 - 1, esm_latitude.shape[0], esm_longitude.shape[0]),
            fill_value=-PG10_C
    )
    ...
    flat10_cdr_data = np.concatenate(
        [decline_section, flat_minus10_section, zero_section],
        axis=0
    )
...
def create_f10cdr_cube(flat10_cdr_data, flat10_cdr_times, esm_latitude, esm_longitude):
    """
...
def cmip7_f10cdr_co2_generate(args):
    ...
    f10cdr_data = create_f10cdr_data(args.nyear_zero, esm_latitude, esm_longitude)
    f10cdr_times = create_f10cdr_times(f10cdr_data, args.year_start)
    f10cdr_cube = create_f10cdr_cube(f10cdr_data, f10cdr_times, esm_latitude, esm_longitude)
